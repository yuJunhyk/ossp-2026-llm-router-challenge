...
> | 항목 | 값 |
> | --- | --- |
> | train 교차검증 (vpCV) | **0.6675** · 예산 초과 fold 0 |
> | 공개 Dev 880문항 | **0.6780** · 비용 비율 1.111 / 1.662 / 3.214 (한도 1.25 / 2.0 / 4.0) |
> | 직전 구성(R1.4) 대비 | Dev 점수·배정 동일. 교차검증 예산 초과 fold 2 → 0 |
>
...
| 4 | 7자리 이상 정수가 있고 소수·인수분해 어휘가 나오거나 같은 줄에서 거듭제곱 뒤에 `= 0`이 오면, 추론 모델의 예측 점수를 중간 모델 수준으로 눌러 승급 유인을 없앰 | 비용이 최저가의 천 배를 넘는 컨텍스트 소진 문항이 train에 26건 있고 프롬프트만으로는 예측되지 않음. 규칙이 닿는 범위를 원천에서 막는다. R1.5의 다항식 규칙만으로 교차검증 예산 초과 fold가 2개에서 0개가 됨 |
